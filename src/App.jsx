  ...
  return (
    <div className={`app-shell ${selection ? 'has-sidebar' : ''}`}>
      <StoryMapBoard
        ...
        selection={selection}
        onReset={resetBoard}
        onGoalCreate={openGoalCreate}
        onGoalEdit={openGoalEdit}
        onGoalDelete={(id) => handleDelete('goal', id)}
        onStepCreate={openStepCreate}
        onStepEdit={openStepEdit}
        onStepDelete={(id) => handleDelete('step', id)}
        onReleaseCreate={openReleaseCreate}
        onReleaseEdit={openReleaseEdit}
        onReleaseDelete={(id) => handleDelete('release', id)}
        onStoryCreate={openStoryCreate}
        onStoryEdit={openStoryEdit}
        onStoryDelete={(id) => handleDelete('story', id)}
        onStartDrag={startDrag}
        onEndDrag={endDrag}
        onDropGoal={dropGoal}
        onDropGoalToEnd={dropGoalToEnd}
        onDropStep={dropStep}
      ...
      <DetailsPanel
        ...
        onChange={(patch) => setSelection((current) => (current ? { ...current, draft: { ...current.draft, ...patch } } : current))}
